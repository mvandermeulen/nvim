...
Path.root_patterns = { ".git", ".is_root_directory", "lua", "package.json", "mvnw", "gradlew", "pom.xml", "build.gradle", "release", ".project" }
...
function Path.root()
  ...
  if not root then
    ...
    ---@type string?
    root = vim.fs.find(Path.root_patterns, { path = path, upward = true })[1]
