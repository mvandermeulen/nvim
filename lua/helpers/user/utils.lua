...
-- Merged contents of user/tools.lua

local _name = 'user.utils'
...
function M.get_home()
  ...
  if not home then
    mlog("Could not determine home directory", "error")
    return nil
  end
  return home
end
...
M.config = {
  socket_path = M.get_home() .. '/.local/state/snuggle.socket',-- default socket path
}
