...
M.basic = function()
  ...
  map('c', mappings.command_nav.expand_directory, [[<C-R>=expand("%:p:h") . "/" <CR>]], { silent = false })
  --
  -- -- MACROS
  map(
    'x',
  ...
  --
  -- -- ToggleChar
  map('n', mappings.plugins.toggle_character.toggle_comma, ":lua require('utils').toggle_char(',')<CR>")
  map('n', mappings.plugins.toggle_character.toggle_semicolon, ":lua require('utils').toggle_char(';')<CR>")
  ...
  -- Cheatsheet
  map('n', mappings.misc.cheatsheet, ":lua require('utils.cheatsheet').show(require('core.keymaps').mappings)<CR>")

  -- This line opens my plugins file in a vertical split
  map('n', '<leader>ep', string.format('<Cmd>vsplit %s/lua/plugins/init.lua<CR>', vim.fn.stdpath 'config'))
