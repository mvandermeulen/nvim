...
class TestSessionStartHook:
  ...
  def test_session_start_runs_git_status(self):
    ...
    test_input = {"session_id": "session-789", "source": "startup"}
    ...
    assert any(
      word in output_lower for word in ["git", "status", "branch", "changes"]
    )
  ...
  def test_session_start_without_git_flag_works_normally(self):
    ...
    # Arrange
    hook_path = Path(__file__).parent / "session_start.py"
    test_input = {"session_id": "session-101112", "source": "resume"}
...
class TestHooksIntegration:
  ...
  def test_both_hooks_can_run_independently(self):
    ...
    session_start_input = {"session_id": "integration-test-2", "source": "startup"}
