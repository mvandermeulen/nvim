...
import subprocess
import os
from agentharness.utils.trace_decision import log_decision
...
completion_checks_enabled = preferences.get("completion_checks", {}).get(
  "enabled", True
)
automated_commits_enabled = preferences.get("automated_git_commits", {}).get(
  "enabled", True
)
...
try:
  ...
  if staged_result.returncode != 0:
    ...
    all_modified_files.extend(
      [f for f in staged_file_list if not should_ignore_file(f)]
    )
  ...
  if unstaged_result.returncode != 0:
    ...
    unstaged_file_list = (
      ...
      if unstaged_files.stdout.strip()
      else []
    )
    all_modified_files.extend(
      [f for f in unstaged_file_list if not should_ignore_file(f)]
    )
  ...
  if untracked_result.stdout.strip():
    ...
    all_modified_files.extend(
      [f for f in untracked_files if not should_ignore_file(f)]
    )
  ...
  if has_remaining_changes:
    ...
    if history_exists or specs_exists:
      ...
      if has_remaining_changes and (
        (history_exists and not history_modified)
        or (specs_exists and not specs_modified)
      ):
        missing_updates = []
